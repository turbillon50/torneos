...
export default function PlantillaPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/teams').then(r => r.json()).then((teams: Team[]) => {
      ...
      setTeam(myTeam || null)
      if (myTeam) {
        fetch(`/api/players?team_id=${myTeam.id}`)
          .then(r => r.json()).then(p => { setPlayers(p); setLoading(false) })
  ...
  async function addPlayer() {
    ...
    const res = await fetch('/api/players', {
      ...
      body: JSON.stringify({ team_id: team.id, name: form.name, number: form.number ? parseInt(form.number) : null, position: form.position || null })
    })
  ...
  if (!team) return <div style={{ textAlign: 'center', padding: 60, color: '#555' }}>Sin equipo asignado.</div>
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          ...
          <div style={{ color: '#888', fontSize: 13 }}>{team.name} · {players.length} jugadores</div>
        </div>
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 1fr', gap: 8, marginBottom: 12 }}>
              <input placeholder="Nombre del jugador" value={form.name} onChange={e => setForm(p => ({...p, name: e.target.value}))} style={{
                background: '#0d0d0d', border: '1px solid #333', borderRadius: 8, padding: '10px 12px',
                color: '#fff', fontSize: 14, outline: 'none'
              }} />
              <input placeholder="#" type="number" value={form.number} onChange={e => setForm(p => ({...p, number: e.target.value}))} style={{
                background: '#0d0d0d', border: '1px solid #333', borderRadius: 8, padding: '10px 8px',
                color: '#fff', fontSize: 14, outline: 'none', textAlign: 'center'
              }} />
              <select value={form.position} onChange={e => setForm(p => ({...p, position: e.target.value}))} style={{
                background: '#0d0d0d', border: '1px solid #333', borderRadius: 8, padding: '10px 12px',
                color: form.position ? '#fff' : '#555', fontSize: 14, outline: 'none'
              }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              }}>{saving ? 'Guardando...' : 'Guardar Jugador'}</motion.button>
      ...
      {players.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 60, color: '#555' }}>
          ...
          <div>Aún no hay jugadores. Agrega el primero.</div>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {players.map((p, i) => (
            <motion.div key={p.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0, transition: { delay: i * 0.03 } }}
              ...
              <div style={{ width: 32, height: 32, background: '#222', borderRadius: 8,
                display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: 14, color: '#CCFF00' }}>
                {p.number || '—'}
              </div>
              <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 8, fontSize: 12 }}>
                ...
                {p.status === 'suspended' && <span style={{ background: 'rgba(255,68,68,0.1)', color: '#FF4444', padding: '2px 8px', borderRadius: 999, fontSize: 10 }}>SUSPENDIDO</span>}
              </div>
            </motion.div>
          ))}
        </div>
